...
**Does `Player` belong in `TitanGame` at all?** Its three fields answer differently:
...
So the current placement is fine, and the id pattern comes out of the question stronger: entities reference a seat id, game state resolves it for `score`, and a future session roster resolves it for display identity. The eventual multiplayer refinement is that `Player` slims to seat-keyed rules state while `name` migrates to the roster — not that id lookups go away.
