...
Doc 01 (toolchain) is done except one open item: pnpm is still pinned to 8.15.0 (`packageManager` in `package.json`), short of the proposed 11.x. Doc 02 (CI and deployment) is fully landed; the doc has been removed. Doc 06 (UI chrome) is fully landed; the doc has been removed. Doc 08 (bundle size) is resolved except one open item: a Vite/Rolldown chunk-duplication bug in dice-box's `world.offscreen.js` build output, non-urgent since lazy-loading already keeps that chunk off the critical path.
