...
| `masterboard/initiateBattle` | Reveals both stacks to the opponent, called once per battle. Rule 9.1 has the active player choose engagement order only after inspecting each defending stack — a choice `finalizeMoves` can't make since it commits before any stack is revealed — and since a round can engage more than one stack at once (`getEngagedStacks`) and `finalizeMoves` no longer opens the first automatically (#256), the player picks from a per-stack list, so this stays a separate action rather than folding into `finalizeMoves`. |
