<template>
  ...
  >
    ...
    />
    <v-card-text>
      Rolled {{ activeStrike.rolls.length }} {{ activeStrike.rolls.length === 1 ? "die" : "dice" }}, needed
      {{ activeStrike.toHit }}s or better to hit.
    </v-card-text>
    ...
    >
      ...
      }}{{ ActiveStrike.isRangestrike(activeStrike) ? " with a rangestrike" : "" }}.
      <span v-if="creature.wounds >= creature.strength">The {{ creature.name }} is dead.</span>
    </v-card-item>
    <v-card-text>
      ...
      <span v-else-if="!ActiveStrike.isRangestrike(activeStrike)">No hits remain to carry over.</span>
    </v-card-text>
...
const targets = computed((): [BattleCreature, number][] =>
  ActiveStrike.targets(props.activeStrike).map(([hex, hits]): [BattleCreature, number] => [
    Battle.creatureOnHex(props.battle, hex)!,
    hits,
  ]),
)
