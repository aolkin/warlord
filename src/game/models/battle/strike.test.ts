...
describe("StrikeOps.combine", () => {
  it("sums dice and toHit across two strike adjustments", () => {
    expect(StrikeOps.combine({ toHit: 3, dice: 2 }, { toHit: 0, dice: 2 })).toEqual({ toHit: 3, dice: 4 })
  })
  ...
  it("clamps a combined toHit above 6 down to 6, since no die roll could ever hit a higher number", () => {
    ...
    expect(StrikeOps.combine({ toHit: 6, dice: 0 }, { toHit: 1, dice: 0 })).toEqual({ toHit: 6, dice: 0 })
  })
  ...
  it("skips clamping when clampVal is explicitly false", () => {
    ...
    expect(StrikeOps.combine({ toHit: -3, dice: 0 }, { toHit: 0, dice: 0 }, false)).toEqual({ toHit: -3, dice: 0 })
    expect(StrikeOps.combine({ toHit: -3, dice: 0 }, { toHit: 0, dice: 0 })).toEqual({ toHit: 2, dice: 0 })
  })
})
