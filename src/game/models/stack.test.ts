...
import {
  canMuster, createStack, finalizeSplit, getCreaturesSplit, isValidSplit, MusterBasis, musterable, numSplitting
} from "./stack"
...
describe("Stack splitting", () => {
  it("rejects a first-round split unless exactly 4 creatures split off with exactly one lord", () => {
    const stack = createStack(PlayerId.RED, 5, 0, 0)
    // Default 8: [TITAN, ANGEL, CENTAUR, CENTAUR, OGRE, OGRE, GARGOYLE, GARGOYLE]
    expect(isValidSplit(stack, true)).toBe(false) // nothing marked yet

    stack.split[0] = true // TITAN
    stack.split[2] = true // CENTAUR
    stack.split[4] = true // OGRE
    expect(isValidSplit(stack, true)).toBe(false) // only 3 marked

    stack.split[1] = true // ANGEL - now 4 marked but 2 lords (TITAN + ANGEL)
    expect(isValidSplit(stack, true)).toBe(false)

    stack.split[1] = false
    stack.split[6] = true // GARGOYLE - back to 4 marked, 1 lord (TITAN)
    expect(isValidSplit(stack, true)).toBe(true)
  })

  it("allows splitting off zero, or two-or-more leaving at least two behind, in later rounds", () => {
    const stack = createStack(PlayerId.RED, 5, 0, 0)
    expect(isValidSplit(stack)).toBe(true) // no pending split is always valid outside round 1

    stack.split[0] = true
    expect(isValidSplit(stack)).toBe(false) // splitting exactly 1 creature is never valid

    stack.split[1] = true
    expect(isValidSplit(stack)).toBe(true) // 2 splitting, 6 remaining

    // Splitting off 6 of 8 would leave only 2 behind, which is still valid...
    for (let i = 2; i < 6; i++) stack.split[i] = true
    expect(numSplitting(stack)).toBe(6)
    expect(isValidSplit(stack)).toBe(true)

    // ...but leaving fewer than 2 behind is not.
    stack.split[6] = true
    expect(numSplitting(stack)).toBe(7)
    expect(isValidSplit(stack)).toBe(false)
  })
  ...
  it("finalizes a split into a new stack sharing the same hex but a new marker", () => {
    ...
    stack.split[4] = true
    stack.split[6] = true
    const splitOff = getCreaturesSplit(stack, true)
    ...
    expect(stack.creatures).toEqual(getCreaturesSplit(stack, false))
    // The pending split markers are cleared after finalizing.
    expect(numSplitting(stack)).toBe(0)
  })
...
describe("Stack mustering eligibility", () => {
  it.each([
    ...
    const stack = createStack(PlayerId.RED, 5, 0, 0, new Array(creatureCount).fill(CreatureType.CENTAUR))
    if (moved) stack.hex = 6

    expect(canMuster(stack)).toBe(expected)
  })
...
describe("Stack musterable (recruit-tier constraints)", () => {
  it.each([
    ...
    const stack = createStack(PlayerId.RED, 5, 0, 0, new Array(centaurCount).fill(CreatureType.CENTAUR))
    const possibilities = musterable(stack, Terrain.PLAINS)
  ...
  it("still locks the Ranger tier behind Lions specifically, even once Centaurs unlock the Lion tier", () => {
    ...
    const stack = createStack(PlayerId.RED, 5, 0, 0, [CreatureType.CENTAUR, CreatureType.CENTAUR])
    const [, rangerBases] = musterable(stack, Terrain.PLAINS).find(([type]) => type === CreatureType.RANGER)!
    expect(rangerBases).toEqual([])
  })

  it("always offers the tower's basic recruits regardless of stack contents", () => {
    const stack = createStack(PlayerId.RED, 100, 0, 0, [CreatureType.CENTAUR])
    const possibilities = musterable(stack, Terrain.TOWER)
  ...
  it("only offers a Guardian once 3 of any one creature type are present", () => {
    const shortOfThree = createStack(PlayerId.RED, 100, 0, 0, [CreatureType.CENTAUR, CreatureType.CENTAUR])
    const [, noGuardian] = musterable(shortOfThree, Terrain.TOWER).find(([t]) => t === CreatureType.GUARDIAN)!
    expect(noGuardian).toEqual([])

    const threeOfAKind = createStack(PlayerId.RED, 100, 0, 0,
      [CreatureType.CENTAUR, CreatureType.CENTAUR, CreatureType.CENTAUR])
    const [, guardianBases] = musterable(threeOfAKind, Terrain.TOWER).find(([t]) => t === CreatureType.GUARDIAN)!
    expect(guardianBases).toEqual([[CreatureType.CENTAUR, 3]])
  })

  it("only offers a Warlock when a Titan is present", () => {
    const noTitan = createStack(PlayerId.RED, 100, 0, 0, [CreatureType.CENTAUR])
    const [, noWarlock] = musterable(noTitan, Terrain.TOWER).find(([t]) => t === CreatureType.WARLOCK)!
    expect(noWarlock).toEqual([])

    const withTitan = createStack(PlayerId.RED, 100, 0, 0, [CreatureType.TITAN])
    const [, warlockBases] = musterable(withTitan, Terrain.TOWER).find(([t]) => t === CreatureType.WARLOCK)!
